...
import { GetEventDocument, AddFeedbackDocument } from '../../src/generated/graphql'
...
export default function AddFeedbackPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Link to={`/events/${id}`} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ChevronLeft size={16} /> {event.name}
      </Link>

      <h1 className="text-3xl font-bold">Submit feedback for &apos;{event.name}&apos;</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md">
        ...
        <button type="submit" className="bg-primary text-primary-foreground rounded px-4 py-2 text-sm font-medium w-fit">
          Submit
        </button>
      </form>
    </div>
